...
def simple_arg_format(value, expected_types, default):
    ...
    Args:
      ...
      default: default value to be set in the event that value is not of expected_types

    Returns:
      type: value if matching expected types otherwise default
    """
    log.debug("simple_arg_format: {} - {} - {}".format(value, expected_types, default))
    if isinstance(value, expected_types):
        log.debug("{} matches type(s) {}".format(value, expected_types))
        return value
    log.warning("{} did not match {}, returning {}".format(value, expected_types, default))
    return default


def list_arg_format(list_of_values, expected_types):
    """
    Check that each value in `list_of_values` matches the expected_types. If the list of values
    is a primitive or simple type, it will be wrapped in a list and returned.

    Args:
      list_of_values: List (or primitive type) where the values will be checked. When the values 
      are of the expected type, they are added to a returned list.
    
      expected_types (tuple): tuple of types to be used for verification 

    Returns:
      list: All values that matched the expected_types will be added to the returned list.
    """
    log.debug("list_arg_format: {} - {}".format(list_of_values, expected_types))
    if isinstance(list_of_values, (int, float, str, bool)):
        log.warning("{} is a primitive type, sending to simple_arg_format".format(list_of_values))
        simple_output = simple_arg_format(list_of_values, expected_types, None)
...
def check_args(*args, **kwargs):
    """
    Check the arguments to ensure that the following parameters are passed and contain the correct type.
    If the type is not correctly formatted and can be formatted it will be altered, otherwise the data
    is reset to a proper empty value. 
    ...
    Returns:
      dict: dictionary formatted with the arguments above, if they did not exist in `kwargs`, defaults
            will be applied.
    """
    return {
        ...
        "SOURCE_DIR": simple_arg_format(kwargs.get("SOURCE_DIR", "."), str, "."),
        "BUILD_DIR": simple_arg_format(kwargs.get("BUILD_DIR", "docs"), str, "docs")
    }
